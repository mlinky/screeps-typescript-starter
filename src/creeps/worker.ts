...
export class CreepWorker extends MyCreep {
    ...
    private newTask() {
        ...
        if (this.creep.carry.energy > 0) {
            ...
            let onlyUpgrade: boolean = false;
            if (this.workRoom === this.homeRoom && Game.rooms[this.homeRoom].controller && Game.rooms[this.homeRoom].controller!.level === 1) {
                onlyUpgrade = true;
            }
        ...
        function assignTaskInRoom(creep: CreepWorker, room: string): boolean {
            ...
            // Repair high priority stuff
            if (repairStructures(0.4)) { return true };
            ...
            // Repair high priority stuff
            if (repairStructures(0.8)) { return true };

            return false;

            function repairStructures(damage: number): boolean {
                // Repair stuff?
                const targets = Game.rooms[room].find(FIND_STRUCTURES, {
                    filter: object => object.hits < object.hitsMax * damage
                });
